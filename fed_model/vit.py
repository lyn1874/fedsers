...
class MultiHeadAttention(nn.Module):
    def __init__(self, input_dim, num_heads=8, dim_head=64, dropout_rate=0.0):
        """Args:
        input_dim:
...
class Transformer(nn.Module):
    def __init__(self, input_dim, depth, num_heads, dim_head, mlp_dim, dropout_rate=0.0):
        super().__init__()
        layers = nn.ModuleList([])
        for i in range(depth):
            layers.append(nn.ModuleList([
                PreNorm(input_dim, MultiHeadAttention(input_dim, num_heads, dim_head, dropout_rate)),
                PreNorm(input_dim, MLPBlock(input_dim, mlp_dim, dropout_rate))
            ]))
        self.layers = layers
...
class ViT(nn.Module):
    def __init__(self, image_size=(30, 30), patch_size=(2, 2), num_classes=2, input_dim=192, depth=2, num_heads=3, mlp_dim=512, pool="cls",
                 channels=110, dim_head=64, dropout=0.0, emb_dropout=0.0, 
        ...
        self.transformer = Transformer(input_dim, depth, num_heads, dim_head, mlp_dim, dropout)
    ...
    def forward_test(self, img):
        ...
        return x, pred, query_key_value, attention_maps
