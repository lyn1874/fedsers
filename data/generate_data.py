...
class SERSGeneratorMore(object):
    ...
    def simulate_hotspot(self):
        ...
        #     # _bot = (3 ** 2 * self.hotspot_size ** 2)
        #     # _up = (- self.hotspot_size ** 2 + 3 ** 2)
        #     A = A * 10
        # #     # A = A * np.exp(-(self.hotspot_size / 3) )
        # #     # A = A * np.exp(1 / (self.hotspot_size / 3))
        #     print("The udpated A", A)
            # A = abs(np.random.normal(self.sbr, 0.05, [self.num_hotspot, 1])) #/ self.num_hotspot
        # A = np.ones([self.num_hotspot, 1])
        # A = np.random.rand(self.num_hotspot, 1) + 0.5 # what is this though? The scale of the hotspots
        if self.fix_hotspot_intensity:
            if self.sbr == 0:
